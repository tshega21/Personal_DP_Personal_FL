...
save_file = os.path.join(save_dir, f"{args.data_type}_results_ditto_dp_rpdp.csv")
...
for ename in epsilons:
    ...
    # DR TRUEX THINKS THIS WILL GIVE YOU 4.3 pt 2
    # full_epsilons = np.array(BoundedFunc(GENERATE_EPSILONS_FUNC[name](len(sorted_labels), SETTINGS[name][int(p_id)])))
    # full_epsilons.sort()
    # target_epsilons= np.array([full_epsilons[sorted_labels[1]==i] for i in NUM_CLIENTS])
    ...
    privacy_engine.prepare_fedrpdp(
        num_steps = NUM_STEPS + args.num_personal_steps,
        num_rounds = NUM_ROUNDS,
        client_rate = CLIENT_RATE,
        ...
        client_labels = sorted_labels
    )
    # If this is present Ditto Personal Training IS Differentially Private
    privacy_engine_ditto = PrivacyEngine(accountant="fed_rdp", n_clients=NUM_CLIENTS)
    privacy_engine_ditto.prepare_fedrpdp(
        num_steps = NUM_STEPS + args.num_personal_steps,
        num_rounds = NUM_ROUNDS,
        client_rate = CLIENT_RATE,
        target_epsilons = target_epsilons,
    ...
    current_args["privacy_engine_ditto"] = privacy_engine_ditto
    ...
    for key in ["global", "personal", "pooled"]:
        ...
        print(f"Mean performance of rpdp ditto dp {key}, Perf={mean_perf:.4f}")
    ...
    del privacy_engine, privacy_engine_ditto, s, cm
...
"""
    ...
    record_personal = [{
      ...
      # prepare global results
    record_pooled = [{
        "perf": str(perf_pooled),  # store as string
    ...
    del privacy_engine, privacy_engine_ditto, s, cm, mean_perf_personal, mean_perf_global,  mean_perf_pooled

    
    # We run Ditto with rPDP (StrongForAll)
    current_args = copy.deepcopy(training_args)
    current_args["num_personal_steps"] =   5
    ...
    privacy_engine.prepare_feddp(
        num_steps = NUM_STEPS + current_args["num_personal_steps"]
        num_rounds = NUM_ROUNDS,
    ...
    privacy_engine_ditto = PrivacyEngine(accountant="fed_rdp", n_clients=NUM_CLIENTS)
    privacy_engine_ditto.prepare_feddp(
        num_steps = NUM_STEPS,
        num_rounds = NUM_ROUNDS + current_args["num_personal_steps"],
        sample_rate = BATCH_SIZE / min([len(train_dl.dataset) for train_dl in training_dls]),
        client_rate = CLIENT_RATE,
        target_epsilon = min_epsilon,
        target_delta = TARGET_DELTA,
    ...
    mean_perf_pooled = np.mean(perf_pooled[-3:])
    ...
    print(f"Mean pooled personal performance of StrongForAll, eps={min_epsilon}, delta={TARGET_DELTA}, Perf={mean_perf_pooled:.4f}")
