...
save_file = os.path.join(save_dir, f"{args.data_type}_results_ditto_dp_unidp_{args.dataset}.csv")
...
if args.reg_param == 0:
    privacy_num_steps_g = NUM_STEPS
    privacy_num_steps_p = args.num_personal_steps
else:
    privacy_num_steps_g = NUM_STEPS + args.num_personal_steps
    privacy_num_steps_p = NUM_STEPS + args.num_personal_steps
...
for key in ["global", "personal", "pooled"]:
    ...
    print(f"Mean performance of unidp ditto dp {key}, Perf={mean_perf:.4f}")
