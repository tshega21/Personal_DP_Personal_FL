...
F_MNIST_MEAN = 0.5
F_MNIST_STD = 0.5
F_MNIST_TRANSFORM = Compose([ToTensor(), Normalize(F_MNIST_MEAN, F_MNIST_STD)])
...
class FashionMnistRaw(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            X = F_MNIST_TRANSFORM(X)
        return X, y


class FedFashionMnist(FashionMnistRaw):
    def __init__(
        self,
        rawdata: FashionMnistRaw,
        center: int = 0,
        train: bool = True,
        pooled: bool = False,
        transform: bool = True
