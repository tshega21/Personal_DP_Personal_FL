...
class MR_MTL:
    ...
    def __init__(
        ...
        privacy_engine_mr_mtl: PrivacyEngine = None,
        device: str = "cuda:0",
        log: bool = False,
        log_period: int = 100,
        ...
        **kwargs
        ...
        self.pprivacy_engine_mr_mtl = privacy_engine_mr_mtl
        ...
        if self.privacy_engine_mr_mtl is not None: 
            assert (self.privacy_engine_mr_mtl.accountant.mechanism() == "idp"), \
                "DataType of `privacy_engine.accountant` must be `IndividualAccountant` in FL setup."
            for p_model in self.personal_models_list:
                p_model._make_private(self.privacy_engine_mr_mtl)
    ...
    def _personal_optimization(self, local_model: _Model, personal_model: _Model):
        """Carry out the local optimization step."""
        if self.privacy_engine_mr_mtl is None:
            local_model._mr_mtl_local_train(personal_model,\
                                    self.num_personal_steps, self.reg_param)
            
        # privacy engine exists that is not idp (multiple accountants)
        elif not (self.privacy_engine_mr_mtl.accountant.mechanism() == "idp"):
            local_model._mr_mtl_local_train(personal_model, \
                                self.num_personal_steps, self.reg_param, \
                                privacy_accountant=self.privacy_engine_mr_mtl.accountant)
        ...
        #DO I ATTACH PRIVACY ENGINE TO LOCAL OR PERSONAL ***
            ...
            local_model._mr_mtl_local_train(personal_model, \
                               self.num_personal_steps, self.reg_param,\
                               privacy_accountant=self.privacy_engine_mr_mtl.accountant.accountants[personal_model.client_id] )
    ...
    def run(self):
        ...
        for r in range(self.num_rounds):
            ...
            print(f"Round={r}, personal pooled={list(perf_pooled.values())}, mean perf={correct_personal_pooled}/{total_personal_pooled} ({correct_personal_pooled/total_personal_pooled:.4f}%)")
